...
PROVIDER_PRESETS: dict[str, dict[str, Any]] = {
    "requesty": {"base_url": "https://router.requesty.ai/v1", "key_envs": ("REQUESTY_API_KEY",), "model": "openai/gpt-4o-mini"},
    "openrouter": {"base_url": "https://openrouter.ai/api/v1", "key_envs": ("OPENROUTER_API_KEY",), "model": "openai/gpt-4o-mini"},
    "groq": {"base_url": "https://api.groq.com/openai/v1", "key_envs": ("GROQ_API_KEY",), "model": "llama-3.3-70b-versatile"},
    "gemini": {"base_url": "https://generativelanguage.googleapis.com/v1beta/openai", "key_envs": ("GEMINI_API_KEY", "GOOGLE_API_KEY", "GOOGLE_AI_STUDIO_API_KEY"), "model": "gemini-2.0-flash"},
    "google": {"base_url": "https://generativelanguage.googleapis.com/v1beta/openai", "key_envs": ("GEMINI_API_KEY", "GOOGLE_API_KEY", "GOOGLE_AI_STUDIO_API_KEY"), "model": "gemini-2.0-flash"},
    "google-ai-studio": {"base_url": "https://generativelanguage.googleapis.com/v1beta/openai", "key_envs": ("GEMINI_API_KEY", "GOOGLE_API_KEY", "GOOGLE_AI_STUDIO_API_KEY"), "model": "gemini-2.0-flash"},
    "xai": {"base_url": "https://api.x.ai/v1", "key_envs": ("XAI_API_KEY",), "model": "grok-3-mini"},
}


class MockProvider:
    def complete(self, *, messages: list[dict[str, Any]], tools: list[dict[str, Any]], on_text: Callable[[str], None] | None = None) -> ProviderReply:
        del tools
...
def bounded_env_int(name: str, default: int, minimum: int, maximum: int) -> int:
    ...
    if raw is None or not raw.isdigit(): return default
    return max(minimum, min(maximum, int(raw)))


def optional_bounded_env_int(name: str, minimum: int, maximum: int) -> int | None:
    raw = os.environ.get(name)
    if raw is None or not raw.isdigit(): return None
    return max(minimum, min(maximum, int(raw)))
...
class OpenAICompatibleProvider:
    def __init__(self, *, api_key: str, base_url: str, model: str, timeout: float = 90.0, max_tokens: int | None = None, reasoning_effort: str | None = None, max_retries: int = 2, headers: dict[str, str] | None = None, token_parameter: Literal["auto", "max_tokens", "max_completion_tokens"] = "auto") -> None:
        if not api_key: raise ValueError("FORGE_API_KEY or OPENAI_API_KEY is required for the OpenAI-compatible provider")
        self.api_key, self.base_url, self.model = api_key, base_url.rstrip("/"), model
        self.timeout, self.max_tokens, self.reasoning_effort = timeout, max_tokens, reasoning_effort
        self.max_retries = max(0, min(max_retries, 5)); self.token_parameter = token_parameter
        self.headers = {key: value[:500] for key, value in (headers or {}).items() if value and "\n" not in value and "\r" not in value}

    def complete(self, *, messages: list[dict[str, Any]], tools: list[dict[str, Any]], on_text: Callable[[str], None] | None = None) -> ProviderReply:
        tool_name_map = self._tool_name_map(tools)
        request_messages, request_tools = self._encode_messages(messages, tool_name_map), self._encode_tools(tools, tool_name_map)
        stream_enabled = bool(on_text) and os.environ.get("FORGE_STREAM", "auto").strip().lower() not in {"0", "false", "no", "off"}
        body: dict[str, Any] = {"model": self.model, "messages": request_messages, "temperature": 0, "stream": stream_enabled}
    ...
    def _tool_name_map(tools: list[dict[str, Any]]) -> dict[str, str]:
        ...
        for tool in tools:
            ...
            if not isinstance(function, dict) or not isinstance(function.get("name"), str): continue
            original = function["name"]; encoded = original
            if not re.fullmatch(r"[A-Za-z0-9_-]+", encoded):
                encoded = "forge_" + "".join(c if re.fullmatch(r"[A-Za-z0-9_-]", c) else f"_u{ord(c):x}_" for c in original)
            if encoded in used and result.get(original) != encoded: encoded = f"{encoded}_{len(used)}"
            result[original] = encoded; used.add(encoded)
        return result
    ...
    def _encode_tools(tools: list[dict[str, Any]], tool_name_map: dict[str, str]) -> list[dict[str, Any]]:
        encoded_tools = []
        for tool in tools:
            encoded = dict(tool); function = tool.get("function")
            if isinstance(function, dict):
                encoded_function = dict(function); name = encoded_function.get("name")
                if isinstance(name, str): encoded_function["name"] = tool_name_map.get(name, name)
    ...
    def _encode_messages(messages: list[dict[str, Any]], tool_name_map: dict[str, str]) -> list[dict[str, Any]]:
        ...
        for message in messages:
            encoded = dict(message); calls = message.get("tool_calls")
            if isinstance(calls, list):
                encoded_calls = []
                for call in calls:
                    encoded_call = dict(call); function = call.get("function")
                    if isinstance(function, dict):
                        encoded_function = dict(function); name = encoded_function.get("name")
                        if isinstance(name, str): encoded_function["name"] = tool_name_map.get(name, name)
    ...
    def _restore_message_tool_names(message: dict[str, Any], tool_name_map: dict[str, str]) -> dict[str, Any]:
        restored = dict(message); reverse = {encoded: original for original, encoded in tool_name_map.items()}
        calls = message.get("tool_calls")
        if isinstance(calls, list):
            restored_calls = []
            for call in calls:
                restored_call = dict(call); function = call.get("function")
                if isinstance(function, dict):
                    restored_function = dict(function); name = restored_function.get("name")
                    if isinstance(name, str): restored_function["name"] = reverse.get(name, name)
    ...
    def _read_stream(self, response: Any, on_text: Callable[[str], None], tool_name_map: dict[str, str] | None = None) -> ProviderReply:
        text_parts, tool_fragments, usage = [], {}, {}
        for raw_line in response:
            ...
            delta = choices[0].get("delta") or {}; fragment = delta.get("content")
            if isinstance(fragment, str) and fragment: text_parts.append(fragment); on_text(fragment)
            for call in delta.get("tool_calls") or []:
                index = str(call.get("index", 0)); state = tool_fragments.setdefault(index, {"id": "", "name": "", "arguments": "", "metadata": {}})
                state["id"] += str(call.get("id") or ""); function = call.get("function") or {}
                state["name"] += str(function.get("name") or ""); state["arguments"] += str(function.get("arguments") or "")
                for key in ("extra_content", "thought_signature", "thoughtSignature"):
                    if key in call: state["metadata"][key] = call[key]
        calls = self._parse_tool_fragments(tool_fragments, tool_name_map or {})
        raw_calls = []
        for state in tool_fragments.values():
            raw_call = {"id": state["id"], "type": "function", "function": {"name": self._restore_tool_name(state["name"], tool_name_map or {}), "arguments": state["arguments"] or "{}"}}
            raw_call.update(state["metadata"]); raw_calls.append(raw_call)
        raw_message: dict[str, Any] = {"role": "assistant", "content": "".join(text_parts)}
        if raw_calls: raw_message["tool_calls"] = raw_calls
        return ProviderReply(text="".join(text_parts), tool_calls=calls, usage=usage, raw_message=raw_message if raw_calls else {})
    ...
    def _restore_tool_name(name: str, tool_name_map: dict[str, str]) -> str:
        ...
        exact = reverse.get(name)
        if exact: return exact
        aliases = {re.sub(r"[^A-Za-z0-9_-]", "_", original): original for original in tool_name_map}
        matches = [original for alias, original in aliases.items() if alias == name]
        return matches[0] if len(matches) == 1 else name

    def _parse_payload(self, payload: dict[str, Any], tool_name_map: dict[str, str] | None = None) -> ProviderReply:
        if not isinstance(payload, dict): raise RuntimeError("Provider response must be a JSON object")
        choices = payload.get("choices")
        if not isinstance(choices, list) or not choices or not isinstance(choices[0], dict):
            raise RuntimeError("Provider response is missing a valid choices array")
        message = choices[0].get("message")
        if not isinstance(message, dict): raise RuntimeError("Provider response is missing a valid message object")
        calls: list[ToolCall] = []
        raw_calls = message.get("tool_calls") or []
        if not isinstance(raw_calls, list): raise RuntimeError("Provider tool_calls must be an array")
        for call in raw_calls:
            if not isinstance(call, dict): raise RuntimeError("Provider tool call must be an object")
            function = call.get("function")
            if not isinstance(function, dict): raise RuntimeError("Provider tool call is missing a function object")
            raw_arguments = function.get("arguments", "{}")
            try: arguments = json.loads(raw_arguments) if isinstance(raw_arguments, str) else raw_arguments
            except json.JSONDecodeError as exc: raise RuntimeError(f"Provider returned invalid tool arguments: {exc}") from exc
            if not isinstance(arguments, dict): raise RuntimeError("Provider tool arguments must be a JSON object")
            name = str(function.get("name", "")); call_id = str(call.get("id", ""))
            if not name or not call_id: raise RuntimeError("Provider tool call is missing a name or id")
            calls.append(ToolCall(id=call_id, name=self._restore_tool_name(name, tool_name_map or {}), arguments=arguments))
        return ProviderReply(text=str(message.get("content") or ""), tool_calls=tuple(calls), usage=payload.get("usage") or {}, raw_message=self._restore_message_tool_names(message, tool_name_map or {}))
    ...
    def _parse_tool_fragments(fragments: dict[str, dict[str, Any]], tool_name_map: dict[str, str] | None = None) -> tuple[ToolCall, ...]:
        ...
        for state in fragments.values():
            try: arguments = json.loads(state["arguments"] or "{}")
            except json.JSONDecodeError as exc: raise RuntimeError(f"Provider returned invalid streamed tool arguments: {exc}") from exc
            if not isinstance(arguments, dict): raise RuntimeError("Provider streamed tool arguments must be a JSON object")
            name = OpenAICompatibleProvider._restore_tool_name(state["name"], tool_name_map or {})
            if not name or not state["id"]: raise RuntimeError("Provider streamed tool call is missing a name or id")
            calls.append(ToolCall(id=state["id"], name=name, arguments=arguments))
        return tuple(calls)
...
def build_provider() -> Provider:
    ...
    if provider_name in {"openai", "openai-compatible", "compatible"} or preset:
        ...
        if preset:
            ...
            if not api_key: raise ValueError(f"{provider_name} requires one of: {', '.join(preset['key_envs'])}")
        else:
