...
import { ExternalToolRegistry, loadExternalServers } from "../dist/apps/forge-cli/src/integrations.js";
...
async function fixture() {
  ...
  await writeFile(path.join(root, "package.json"), JSON.stringify({ scripts: { test: "node --version", lint: "node --version", build: "node --version" } }));
  await writeFile(path.join(root, "src", "main.ts"), "export function main() { return true; }\n");
  return root;
}
...
test("context discovers verification commands and symbols", async () => {
  ...
  try {
    ...
    assert.deepEqual(context.verificationCommands, [["npm", "run", "test"], ["npm", "run", "lint"], ["npm", "run", "build"]]);
    assert.ok(context.relevantFiles.some((file) => file.symbols?.includes("main")));
  } finally {
...
test("process failures and timeouts become bounded tool failures", async () => {
  ...
  try {
    ...
    const failure = await tools.execute({ tool: "process.run", arguments: { command: process.execPath, args: ["-e", "process.exit(7)"], timeoutMs: 3000 } });
    assert.equal(failure.ok, false);
    assert.equal(failure.error?.code, "COMMAND_FAILED");
    const timeout = await tools.execute({ tool: "process.run", arguments: { command: process.execPath, args: ["-e", "setTimeout(() => {}, 10000)"], timeoutMs: 100 } });
...
test("checkpoint restoration handles existing and newly-created files", async () => {
  ...
  try {
    ...
    const result = await tools.execute({ tool: "workspace.apply_patch", arguments: { files: [{ path: "README.md", content: "changed\n" }, { path: "created.txt", content: "created\n" }] } });
    ...
    assert.equal(await readFile(path.join(root, "README.md"), "utf8"), "# Stress fixture\n");
...
test("session persistence retains bounded events and rejects invalid IDs", async () => {
  ...
  try {
    ...
    await store.append(record, { ...createEnvelope("agent.text", record.id), type: "agent.text", text: "hello" });
...
test("external server config stays disabled and rejects unsafe IDs", async () => {
  ...
  try {
    ...
    await writeFile(config, JSON.stringify({ servers: [{ id: "helper", command: "helper", args: ["--stdio"], enabled: true }] }));
    const registry = await loadExternalServers(config);
    assert.equal(registry.list()[0].enabled, false);
    assert.throws(() => new ExternalToolRegistry().register({ id: "../bad", command: "helper", args: [], enabled: false, trust: "untrusted", defaultRisk: "network" }), /IDs/);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test("workspace traversal and symlink escapes stay blocked under stress", async () => {
  const root = await fixture();
  const outside = await mkdtemp(path.join(os.tmpdir(), "forge-outside-stress-"));
  try {
    await writeFile(path.join(outside, "secret.txt"), "secret\n");
    await symlink(outside, path.join(root, "external"));
    const tools = new WorkspaceTools(root);
    for (const relativePath of ["../secret.txt", "/etc/passwd", "external/secret.txt"]) {
      const result = await tools.execute({ tool: "workspace.read", arguments: { path: relativePath } });
