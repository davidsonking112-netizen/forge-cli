...
import { AcpJsonlBridge, ExternalToolRegistry } from "../dist/apps/forge-cli/src/integrations.js";
import { McpClientError, McpStdioClient } from "../dist/apps/forge-cli/src/mcp.js";

test("ACP accepts canonical JSON-RPC events and preserves correlation ids", () => {
  const bridge = new AcpJsonlBridge(1_000);
  const response = JSON.parse(bridge.handleLine(JSON.stringify({ jsonrpc: "2.0", id: 7, method: "edit.proposal", params: { workspace: ".", files: ["src/app.ts"], reason: "review" } })));
...
test("ACP rejects malformed, oversized and scalar requests", () => {
  ...
  assert.equal(JSON.parse(bridge.handleLine("not-json")).error.data.category, "parse");
  assert.equal(JSON.parse(bridge.handleLine("x".repeat(101))).error.data.category, "invalid-request");
  assert.equal(JSON.parse(bridge.handleLine(JSON.stringify({ jsonrpc: "2.0", id: 1, method: "prompt", params: "bad" }))).error.code, -32602);
});
...
test("MCP client rejects requests after close", async () => {
  const server = { id: "fixture", command: process.execPath, args: ["-e", "process.stdin.on('data',()=>{})"], enabled: true, explicitConsent: true, trust: "untrusted", defaultRisk: "network" };
  const client = new McpStdioClient(server, 100);
  await assert.rejects(() => client.callTool("tools.call", {}), (error) => error instanceof McpClientError && error.category === "configuration");
  client.close();
  await assert.rejects(() => client.callTool("tools.call", {}), (error) => error instanceof McpClientError && error.category === "configuration");
});
...
test("external tool registry never enables servers implicitly", () => {
  ...
  registry.register({ id: "fixture", command: process.execPath, args: ["-e", ""], enabled: true, trust: "untrusted", defaultRisk: "network" });
