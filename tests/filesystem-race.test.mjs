...
test("workspace writes reject symlink targets without modifying the target", async () => {
  ...
  try {
    ...
    const result = await new WorkspaceTools(dir).execute({ tool: "workspace.apply_patch", arguments: { path: "target.txt", content: "ATTACK" } });
...
test("workspace rejects symlinked parent directories", async () => {
  ...
  try {
    ...
    const result = await new WorkspaceTools(dir).execute({ tool: "workspace.apply_patch", arguments: { path: "link/file.txt", content: "ATTACK" } });
    assert.equal(result.ok, false);
    assert.equal(await readFile(path.join(realDir, "file.txt"), "utf8"), "safe");
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
