...
Mutation proposals are rejected until the supervisor has observed an execution plan and a valid dependency-graph checkpoint. Each graph step records expected files, dependencies, risks, tests, and postconditions. A successful mutation must return changed-file evidence before the run can enter verification. A step remains pending until every referenced prerequisite step is contract-valid and supervisor-verified complete; invalid contracts and dependency cycles block the graph fail closed. For mutation tasks, completion requires a passing targeted check followed by a distinct broad project check, along with the final changed-file list and structured exit-code records. A provider response that says “done” without those artifacts is intercepted; the supervisor sends bounded gate feedback to the active provider session and refuses to accept completion after the bounded rejection ceiling. Read-only tasks use a narrower gate based on inspection or plan evidence.
