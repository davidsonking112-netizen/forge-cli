...
Multi-file edits validate all paths and optional original content hashes before writing. Unified-diff edits additionally validate hunk headers, context lines, line counts, file existence, and rename/delete targets. `forge preview-diff` performs the same bounded read-only analysis and can mark an explicit `--only` file selection. Forge creates a restrictive checkpoint manifest and backups before the selected batch is applied. If any write fails, the supervisor attempts to restore the checkpoint. Users can restore a recorded checkpoint through the `forge undo` command after confirming the target workspace. v0.8 does not claim a general three-way merge engine. Failed or stale changes are rejected before partial writes, and checkpoints remain the recovery boundary.
