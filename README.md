...
Every run is also governed by a supervisor-owned implementation state machine: `intake → inspect → plan → milestone → implement → targeted-verify → evidence → repair/full-verify → summarize`. Each phase exposes its entry conditions, required artifact, exit condition, failure transition, and bounded budget in `agent.state` events. For large tasks, the plan may include a dependency graph. Each proposed checkpoint declares expected files, dependencies, risks, tests, and postconditions; Forge assigns stable step IDs and exposes readiness in `agent.graph` events. A downstream step cannot run while a prerequisite contract is invalid, blocked, or incomplete. A mutation task cannot complete because the provider says “done”: Forge must observe a plan, an approved successful patch with changed-file evidence, a passing targeted check, and a distinct passing broad check. Provider intelligence remains provider-dependent; this machine prevents unsupported success claims and unbounded recovery. The latest state is available in the full-screen TUI, `forge inspect`, JSONL output, and the redacted audit log.
