...
Forge v0.9.0 adds bounded event-specific protocol validation, reproducible verification evidence digests, deterministic change-set digests shared by preview and apply, atomic local-index persistence, read-only extension recipe inspection, stronger worker input bounds, and redacted provider/worker errors. It retains v0.8 recovery, transactional review, local relationships, revocable scopes, categorized local MCP lifecycle errors, correlated ACP responses, and all local-first safety boundaries.
