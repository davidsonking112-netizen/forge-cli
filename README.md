...
Forge v0.99 adds optional Daytona sandbox lifecycle support, explicit context pruning statistics, bounded long-horizon provider continuation, repair-attempt escalation, redacted safety audit logs, and user-configurable system prompts. It retains the visible planning checklist, bounded specialist role contracts, quality-preserving cost profiles, explicit approval-gated GitHub workflows, bounded persisted scratchpad, evidence-aware output optimization prompt, v0.9 protocol validation, reproducible verification evidence digests, deterministic change-set digests, atomic local-index persistence, read-only extension recipe inspection, stronger worker input bounds, and all local-first safety boundaries.
