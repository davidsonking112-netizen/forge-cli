...
Forge v0.9.7 adds visible, bounded planning checklists with user expectations, stronger specialist role contracts, and quality-preserving cost profiles for multi-agent analysis. It retains the explicit, approval-gated GitHub workflows, bounded persisted scratchpad, evidence-aware output optimization prompt, v0.9 protocol validation, reproducible verification evidence digests, deterministic change-set digests, atomic local-index persistence, read-only extension recipe inspection, stronger worker input bounds, and all local-first safety boundaries.
