...
Forge v0.7.0 adds durable step journals, bounded workspace fingerprints for safe recovery, structured verification evidence, read-only conflict-aware diff previews, incremental metadata indexing, policy explanations, narrow expiring approval scopes, and declarative non-executable extension recipes. It retains the v0.6.1 audit hardening and all local-first safety boundaries.
...
Use `forge inspect <session-id>` to view bounded event, timing, approval, delegation, step-journal, fingerprint, and verification metrics. Use `forge verify <session-id>` to inspect structured evidence; it never replays commands and marks passed evidence stale if the workspace fingerprint changed. Resuming a session revalidates the workspace and refuses drift rather than replaying mutations. Use `--no-record` for privacy-sensitive runs, and use `forge git prepare-pr` to generate a local review draft without pushing remotely. `forge doctor --repair` prints setup guidance only; it never installs packages or edits the workspace.

Review and apply a unified diff safely:

```bash
forge review change.patch
forge preview-diff change.patch --workspace /path/to/repository
